...
if (isset($_GET['accountId'])) {
	...
	$sqlaccountId = $con ->query("SELECT accountNumber FROM bank_client WHERE id='$accountId'");
	$fetchAcountName = mysqli_fetch_array($sqlaccountId);
	$accountName = $fetchAcountName['accountNumber'];
}else{
	echo 'no account to show the transactions';
}
...
if (isset($_GET['page'])) {
	...
	$transactions="";
	$sql =$con->query("select * from transactionsview where account_id= '$clientId1' AND (bankId = '$page' AND clientId = '$clientId1')");
	$n=0;
...
<nav class="navbar navbar-default">
  ...
  <div class="collapse navbar-collapse" id="menu">
    <ul class="nav navbar-nav">
      ...
      <li><a href="bankClient.php?page=<?php echo $page;?>&clientId=<?php echo $clientId1;?>"><?php echo $ClientName;?></a></li>
      <li class="active"><a href="javascript:void()"><?php echo $accountName;?></a></li>
    </ul>
    </div>
  </div>
</nav>
	...
	<?php
		if ($countBanks == 1) {
			echo '  <div style="color: #fff; font-size: 20px; background-color: '.$bankcolor.'; height: 100px;">
						...
						&nbsp;&nbsp;&nbsp;('.$n.') transactions of '.$accountName.' from '.$ClientName.'  in '.$bankName.'
					</div>';
		}else{
