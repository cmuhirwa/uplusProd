...
$sql_account = $db->query("SELECT * FROM accounts WHERE groupType LIKE 'wedding' ");
?>
    ...
    <div class="page-content">
      ...
      <div class="panel">
        <header class="panel-heading">
          <h3 class="panel-title">Contribution data</h3>
        </header>
        <div class="panel-body">
          <table class="table table-hover dataTable table-striped width-full" id="exampleTableTools">
            ...
            <?php while($fetch_account = mysqli_fetch_array($sql_account))
            {
              ...
              $Targetacc = number_format($fetch_account['saving']);
              $groupid = $fetch_account['id'];
                $select_gbalance = $outCon->query("SELECT Balance FROM groupbalance WHERE groupId LIKE '$groupid'");
                $fetchgroupbala = mysqli_fetch_array($select_gbalance);
                $balancegroup = number_format($fetchgroupbala['Balance']);
                $currentfees = number_format($balancegroup * 2 / 100);
                $targetfees = number_format($Targetacc * 2 / 100);
            ...
            ?>
              <tr>
                ...
                <td><?php echo $balancegroup?></td>
                <td><?php echo $Targetacc; ?></td>
                <td><?php echo $currentfees; ?></td>
                <td><?php echo $targetfees; ?></td>
                <td>
                  <a href="more group.php?blabla=<?php echo $groupid; ?>"><button class="btn btn-primary">More</button></a>
                </td>
              </tr>
              <?php
          }
