 ...
 // This is called with the results from from FB.getLoginStatus().
  function statusChangeCallback(response) {
    ...
    if (response.status === 'connected') {
	 ...
	 // FB.logout(function(response) {
		//  alert(done);// user is now logged out
	//	});
  ...
  function testAPI() {
    ...
    FB.api('/me', 'GET', {fields: 'first_name,last_name,name,id,gender,picture.with(15).height(150)'}, function(response) {
		...
		document.getElementById('soza').innerHTML = '<button onclick="logoutthis()">logoutthis</button>';
...
<!-- FACEBOOK FILL INFORMATION-->
	...
	<div id="soza">
	Waiting:
	<div id='login'><img src="../assets/images/fb.png" style="cursor: pointer;"/></div>
	</div>
