...
<?php 
	$n=0;
	$sql1 = $db->query("SELECT * FROM `joined_ua` WHERE (`userPhone` = '$phone' and acceptance='no') and groupType='IKIMINA' ");
	$count_savings_alerts = mysqli_num_rows($sql1);
	  
	$sql2 = $db->query("SELECT * FROM `joined_ua` WHERE (`userPhone` = '$phone' and acceptance='no') and groupType='WEDDING'");
	$count_onceoff_alerts = mysqli_num_rows($sql2);
	
	$sql3 = $db->query("SELECT * FROM `joined_ua` WHERE (`userPhone` = '$phone' and acceptance='no') and groupType='TIGHT'");
	$count_reccuring_alerts = mysqli_num_rows($sql3);
	  
	$sql4 = $db->query("SELECT * FROM users WHERE name ='' AND id ='$thisid'");
	$count_profile_alerts = mysqli_num_rows($sql4);
  
	if($count_savings_alerts > 0 || $count_onceoff_alerts >0 || $count_profile_alerts >0){
			...
			while($row = mysqli_fetch_array($sql1))
			{
			  ...
			  if ($acceptance == 'no'){
				echo ' <div class="alert alert-danger alert-dismissible" role="alert">
					...
					<b>'.$groupName.'</b>
					<a data-target="#exampleFillIn" data-toggle="modal" href="javascript:void()" onclick ="group_info(group_id= '.$groupId.', G_personalID='.$thisid.')" data-dismiss="modal">
					Review
					</a>
					 </div>' ;
			  }
		...
		if($count_onceoff_alerts > 0){
			while($row = mysqli_fetch_array($sql2))
			{
			  ...
			  if ($acceptance == 'no'){
				echo ' <div class="alert alert-danger alert-dismissible" role="alert">
					You have invited to contrubute in
					<b>'.$groupName.'</b>
					<a href="onceoff'.$groupId.'")">
					Review
					</a>
